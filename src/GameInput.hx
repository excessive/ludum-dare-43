...
class GameInput {
	...
	static var mouse_mappings = [
		1 => { player: null, menu: null },
		2 => { player: null, menu: Action.MenuToggle }
	];
	...
	static var kb_mappings = [
		...
		KeyConstant.Lshift => { player: Action.Trick, menu: null },
		KeyConstant.Rshift => { player: Action.Trick, menu: null },
	...
	static var gp_mappings = [
		...
		GamepadButton.X => { player: Action.Trick, menu: null },
		GamepadButton.Y => { player: Action.Glide, menu: null },
		...
		GamepadButton.Dpright => { player: null, menu: Action.MenuRight },
	...
	**/
	...
	public static function update(dt: Float) {
		...
		v = gp_raw_axes[GamepadAxis.Triggerleft];
		input_state[Action.LTrigger].value = Math.max(v, 0);

		v = gp_raw_axes[GamepadAxis.Triggerright];
		input_state[Action.RTrigger].value = Math.max(v, 0);
		...
		input_state[Action.LTrigger].press(input_state[Action.LTrigger].value);
		input_state[Action.RTrigger].press(input_state[Action.RTrigger].value);
