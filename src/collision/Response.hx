...
class Response {
	static var VERY_CLOSE_DIST: Float = 0.0025;
	...
	public static function update(position: Vec3, velocity: Vec3, radius: Vec3, gravity: Vec3, query: TriQueryCb, substeps: Int = 1) {
		...
		// thanks exezin for figuring out how to make this behave properly
		// and also debugging practically everything else
		var contacts = [];
		for (i in 0...substeps) {
			var packet: Packet = {
				r3_position: new_position,
				r3_velocity: Vec3.zero(),
				e_radius: radius,
				e_inv_radius: Vec3.rcp(radius),
				e_position: new_position / radius,
			...
			new_position = packet.r3_position;

			// early out: if not moving, no reason to run the remaining iterations.
			if ((new_position - base_position).length() <= VERY_CLOSE_DIST / substeps) {
				// trace('bailed after ${i+1} iters');
				break;
			}
		}
		...
		return {
			...
			velocity: new_position - base_position,
			contacts: contacts
		};
	}
}
