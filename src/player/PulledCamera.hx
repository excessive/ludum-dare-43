...
class PulledCamera {
	public static function update_camera(e: Entity, move: Vec3, dt: Float) {
		...
		var heading_rate = 1.0 + 5.0 * Math.pow(Utils.min(lateral_speed / e.player.speed_limit, 1.0), 2.0);
		var heading_mix = Utils.decay(0, 1, heading_rate, dt);

		var dir = v.copy();
		dir.normalize();
		var pitch_limit = Utils.rad(75);
